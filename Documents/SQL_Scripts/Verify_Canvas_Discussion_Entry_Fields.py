# ===========================================================================================
# Canvas Discussion Entry Fields - Label and Entry Verification Script
# ===========================================================================================
# This script verifies the correctness of data inserted into the `core_label` and 
# `core_entry` tables for Canvas Discussion Entry Fields (table_id_id = 6).
#
# Checks performed:
# - Ensures 34 `core_label` entries exist with the correct `label_index` and `label_name`.
...
#
# Assumptions:
# - `label_id` ranges from 31–40 and 109–132 are used for Canvas Discussion Entry Fields.
...
EXPECTED_FIELDS = [
    'strm', 'class_nbr', 'cls_lms_cd', 'de_created_by_emplid', 'de_created_by_asurite',
    'de_created_by_name', 'de_created_by_most_privileged_role', 'de_created_by_least_privileged_role',
    'de_message_body', 'de_depth', 'de_branch', 'de_primary_id', 'de_message_length',
    'de_created_dttm', 'de_updated_dttm', 'de_deleted_dttm', 'de_state', 'dt_title',
    'dt_type', 'dt_message_body', 'de_created_by_user_id', 'de_id', 'de_parent_id',
    'dt_id', 'course_id', 'dt_user_id', 'dt_editor_id', 'dt_editor_user_combined',
    'dt_assignment_id', 'dt_enrollment_rollup_id', 'dt_actor_type', 'de_replied_to_user_id',
    'group_de_id', 'de_replied_to_de_id'
]

# Expected field_id values in core_entry for each label_id
EXPECTED_FIELD_IDS = list(range(2, 14))  # 2 through 13 inclusive

# Label IDs used in Canvas Discussion Entry Fields (hardcoded)
EXPECTED_LABEL_IDS = list(range(31, 41)) + list(range(109, 133))
